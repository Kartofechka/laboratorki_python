# -*- coding: cp1251 -*-   #Для русского языка в выводе

def converting_temperature (temperature):
    return float(temperature) * 1.8 + 32

try: #обработчик для всех исключений не вдаваясь в тип ошибки
    choice = int(input("""Выберете вариант ввода и вывода данных: 
    [1] Работа с консолью
    [2] Работа с файлом\n""")) #Пользователь сам выбирает способ ввода и вывода данных
    if choice == 1: 
        temperature = int(input("Пожалуйста, введите температуру в градусах Цельсия:\n"))
        print(f"Температура по Фаренгейту = {converting_temperature(temperature)}\n")
    elif choice == 2:
        file_in = open("input.txt", "r")
        temperature = int(file_in.read())
        file_out = open("output.txt", "w", encoding="utf-8") #utf-8 для вывода текста на русском в файл
        file_out.write(f"Температура по Фаренгейту = {converting_temperature(temperature)}\n")
        print("Результат записан в файле \"output.txt\"\n")
        file_in.close()
        file_out.close()
    else:
        print("Неверный вариант")
except Exception as error:
    print("Что-то пошло не так: ", error)
finally: 
    input("Нажмите Enter, чтобы закрыть окно...")
