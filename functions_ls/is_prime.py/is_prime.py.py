...
try: #обработчик для всех исключений не вдаваясь в тип ошибки
    ...
    if choice == 1: 
        number = int(input("Пожалуйста, введите число для проверки на простоту:\n"))
        if is_simple(number):
            print(f"Число {number} - простое\n")
        else:
            print(f"Число {number} - непростое\n")
    elif choice == 2:
        file_in = open("input.txt", "r")
        number = int(file_in.read())
        file_out = open("output.txt", "w", encoding="utf-8") #utf-8 для вывода текста на русском в файл
        if is_simple(number):
            file_out.write(f"Число {number} - простое\n")
        else:
            file_out.write(f"Число {number} - непростое\n")
